...
class TestApartmentMasterService:
    ...
    def test_parse_info_extracts_ratio_fields(self, tmp_db):
        """vlRat(용적률), bcRat(건폐율)이 floor_area_ratio, building_coverage_ratio로 추출된다."""
        from modules.real_estate.apartment_master.service import ApartmentMasterService
        from modules.real_estate.apartment_master.repository import ApartmentMasterRepository
        repo = ApartmentMasterRepository(db_path=tmp_db)
        svc = ApartmentMasterService(client=MagicMock(), repository=repo)
        ...
        info = {
            "hoCnt": 2444, "kaptDongCnt": "36",
            "kaptBcompany": "삼성물산", "kaptUsedate": "20090101",
            "vlRat": "249.98", "bcRat": "19.69",
        }
        master = svc._parse_info("래미안퍼스티지", "11650", "A12345", info)

        assert master.floor_area_ratio == pytest.approx(249.98)
        assert master.building_coverage_ratio == pytest.approx(19.69)

    def test_parse_info_ratio_fields_none_when_missing(self, tmp_db):
        """vlRat/bcRat 없으면 None."""
        from modules.real_estate.apartment_master.service import ApartmentMasterService
        from modules.real_estate.apartment_master.repository import ApartmentMasterRepository
        repo = ApartmentMasterRepository(db_path=tmp_db)
        svc = ApartmentMasterService(client=MagicMock(), repository=repo)
        ...
        assert master.floor_area_ratio is None
        assert master.building_coverage_ratio is None


class TestApartmentMasterRepositoryRatioFields:
    def test_save_and_get_ratio_fields(self, tmp_db):
        """floor_area_ratio, building_coverage_ratio가 저장·조회된다."""
        from modules.real_estate.apartment_master.repository import ApartmentMasterRepository
        from modules.real_estate.models import ApartmentMaster
        repo = ApartmentMasterRepository(db_path=tmp_db)

        master = ApartmentMaster(
            apt_name="래미안퍼스티지", district_code="11650", complex_code="A12345",
            household_count=2444, building_count=36, parking_count=0,
            constructor="삼성물산", approved_date="20090101",
            floor_area_ratio=249.98, building_coverage_ratio=19.69,
        )
        repo.save(master)

        result = repo.get("래미안퍼스티지", "11650")
        assert result is not None
        assert result.floor_area_ratio == pytest.approx(249.98)
        assert result.building_coverage_ratio == pytest.approx(19.69)

    def test_migration_adds_ratio_columns(self, tmp_path):
        """기존 DB에 ratio 컬럼이 없어도 _init_db()가 자동 추가한다."""
        import sqlite3
        db_path = str(tmp_path / "old_no_ratios.db")
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            """)
            conn.execute(
                "INSERT INTO apartment_master (cache_key, household_count) "
                "VALUES ('11650__래미안퍼스티지', 2444)"
            )
            conn.commit()
        ...
        result = repo.get("래미안퍼스티지", "11650")
        assert result is not None
        assert result.floor_area_ratio is None
        assert result.building_coverage_ratio is None
