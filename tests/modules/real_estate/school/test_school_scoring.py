...
DEFAULT_CFG = dict(ideal=12, warning=18, high_count=3, mid_count=1, w_density=0.30, w_quality=0.70)


def test_score_ideal_teacher_ratio_many_schools():
    score = _calc_score(nearby_count=5, avg_per_teacher=10, **DEFAULT_CFG)
    assert score == round(100 * 0.70 + 100 * 0.30)  # 100


def test_score_overcrowded_few_schools():
    score = _calc_score(nearby_count=0, avg_per_teacher=25, **DEFAULT_CFG)
    assert score == round(20 * 0.70 + 20 * 0.30)  # 20


def test_score_medium_teacher_ratio_medium_density():
    # nearby_count=2 >= mid_count=1 but < high_count=3 → density_score=60
    # avg_per_teacher=15 > ideal=12 but <= warning=18 → quality_score=60
    score = _calc_score(nearby_count=2, avg_per_teacher=15, **DEFAULT_CFG)
    assert score == round(60 * 0.70 + 60 * 0.30)  # 60


def test_score_ideal_teacher_no_schools():
    score = _calc_score(nearby_count=0, avg_per_teacher=10, **DEFAULT_CFG)
...
def test_score_weight_sum_is_bounded():
    for nearby in range(0, 6):
        for avg in [10, 15, 25]:
            score = _calc_score(nearby_count=nearby, avg_per_teacher=avg, **DEFAULT_CFG)
            assert 0 <= score <= 100


def test_score_no_quality_data_uses_neutral():
    score = _calc_score(nearby_count=3, avg_per_teacher=0.0, has_quality_data=False, **DEFAULT_CFG)
    # quality_score=50, density_score=100 → round(50*0.70 + 100*0.30) = round(35+30) = 65
