...
1. **Write Spec:** Create `docs/features/{feature_name}/spec.md`. Define goals, architecture, and data models. MUST refer to `docs/master_plan.md` to ensure alignment with the overall vision and architecture.
...
## 4. Workflow Automation (n8n & MCP)
When the user requests a new automated background task (e.g., "Schedule a daily message", "Build a tool to fetch this data every hour"):
1. **Never write raw n8n JSON from scratch.** It leads to broken schemas.
2. **Use Templates:** Look for base configurations in `src/n8n/templates/`. Read the template, duplicate it, and Modify ONLY the specific fields (Endpoint URLs, Cron Schedules, Credentials).
3. **Use MCP Tools:** Use the programmatic tools provided by the FastAPI server (e.g., `deploy_workflow`) to push the generated JSON into the live n8n container. Do not ask the user to copy-paste the JSON manually.
4. **Update Registry:** Always append the newly created routine to `docs/workflows_registry.md` so the system "remembers" it is running in the background.
