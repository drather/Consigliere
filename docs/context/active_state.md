...
### 1순위 — 개별 아파트 심층 분석 기능
- **목표:** 관심 단지를 지정하면 지금까지 수집된 모든 데이터(실거래가 히스토리, 입지점수, 전세가율, 공급리스크, 출퇴근 등)를 통합 분석하는 기능 추가
- **배경:** 현재 Job Report는 당일 실거래가 수신 내역 중심 → 특정 단지를 눈여겨보고 있을 때 축적된 전체 데이터를 기반으로 심층 분석이 필요한 경우 커버 불가
- **개념:**
  - 입력: 단지 코드 or 단지명 (관심 단지)
  - 분석 데이터: 실거래가 히스토리, 전세가율, 공급리스크, 입지점수, 거시경제 지표, 출퇴근
  - 출력: 단지 특화 심층 리포트 (Slack 또는 대시보드)
- **설계 필요** (다음 세션에서 `/brainstorming` 으로 시작)
