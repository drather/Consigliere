...
- **양방향 통신 (Bidirectional)**: 슬랙에서 사용자가 입력한 내용을 n8n 워크플로우 트리거로 연결하는 대화형 기능 구현 (현재 n8n Production Webhook 404 이슈 해결 필요).
- **RAG 고도화**: ChromaDB에 저장된 부동산 데이터를 LLM이 더 정교하게 요약하여 보고할 수 있도록 프롬프트 및 컨텍스트 로직 개선.
