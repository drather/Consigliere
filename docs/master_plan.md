...
| **Core Server**        | FastAPI (Python)                  | 시스템의 두뇌(LLM/Gemini 연동). 클라이언트 요청 해석, 코드/명령어 생성, DB 읽기/쓰기, 워크플로우(n8n) 제어 도구(MCP) 및 **알림(Sender API) 서비스** 제공. |
| **Automation Engine**  | n8n (Node.js)                     | 백그라운드 작업 실행을 담당하는 심장. FastAPI가 짜준 각본(JSON)대로 정해진 시간에 작동하여 외부 API를 호출하거나 **지정된 채널(Slack 등)로 알림**을 전송. |
...
4. **[Execution (n8n)]**:
   - n8n이 크론(Cron) 스케줄러에 따라 외부 API를 찌르고, 그 결과를 **Consigliere API의 Notification Endpoint(/notify/slack)**를 통해 사용자에게 전송.
5. **[Logging]**:
...
- n8n 템플릿 디렉토리 구성 완료.
- FastAPI MCP 라우터 및 `AutomationService` 구현 완료.
- 대시보드 컨테이너화 및 n8n 워크플로우 3종 배포 성공.
- **슬랙 연동 및 알림 레이어(Sender API)** 구축 완료.
- **부동산 뉴스 자동화 스케줄링**(매일 06:00 KST) 및 슬랙 전송 안정화.
- **부동산 모듈 SOLID 리팩토링 및 Scalability 확보** 완료 (2026-03-16).

**[진행 예정 사항 (Immediate Focus)]**
- **양방향 통신 (Bidirectional)**: 슬랙에서 사용자가 입력한 내용을 n8n 워크플로우 트리거로 연결하는 대화형 기능 구현.
- **RAG 고도화**: ChromaDB에 저장된 데이터를 LLM이 더 정교하게 요약하여 보고할 수 있도록 프롬프트 개선.
- **기본 실거래가 모니터링 고도화**: 팩트 추출 기능과 연계한 국토부 API 수집 로직 보완.
